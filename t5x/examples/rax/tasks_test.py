...
# If reproducing, must replace this one
_DATA_DIR = '/store/scratch/m474li/rax_rankt5_repro/msmarco_datasets/tfds'
...
class TasksTest(tf.test.TestCase):
  ...
  def test_example_shard(self):
    # Functionality test to ensure that the TFDS builder is compatible with the MS MARCO processor
    # in tasks.py
    ds = tfds.load('msmarco_datasets', data_dir=_DATA_DIR, split='dev[0:1]')
    tf.random.set_seed(42)
    inputs_mock = mock.Mock()
    inputs_mock.vocabulary.encode_tf.return_value = tf.ragged.constant(
        [[5, 1024, 250, 1, 24, 205, 555], [5, 304, 200, 200],
    ...
    ds = tasks._msmarco_preprocessor(ds, {
        "inputs": inputs_mock,
        "targets": targets_mock,
        "label": label_mock,
        "mask": mask_mock
